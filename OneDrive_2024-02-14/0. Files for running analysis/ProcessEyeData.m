function ProcessEyeData(participants)
...
for s = participants
    ...
    fileName = strcat(int2str(s), '_OUTPUT');
    load(fileName, 'DATA');
    
    DATA_EG_PROC = cell(60,4);
    imgOrder = cell2mat(DATA.imagepair(1));
    ...
    load('StimDetails');
        ...
        for t = 1:60
            ...
            curDetails = StimDetails(StimDetails(:,1)==imgOrder(t),2:3);
            
            fixStore(fixStore(:,3)== 0,:) = [];
            DATA_EG_PROC(t,:) = {[imgOrder(t) curDetails] fixStore EGdata [GapsBeforeFill GapsAfterFill]}; % store in cell

        end
        
        fileName = ['.\Processed DATA\', int2str(s)];
        %fileName = [int2str(s)];
